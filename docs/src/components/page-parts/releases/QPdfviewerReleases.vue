...
export default {
  ...
  setup () {
    ...
    const currentPackage = ref('QPdfviewer')
    const versions = ref({})
    ...
    function queryReleases (page = 1) {
      ...
      xhrQuasar.addEventListener('load', function () {
        ...
        // sort by date
        packages.value.QPdfviewer.sort((a, b) => {
      ...
      xhrQuasar.open('GET', `https://api.github.com/repos/quasarframework/app-extension-qpdfviewer/releases?page=${ page }&per_page=100`)
      xhrQuasar.send()
    }
